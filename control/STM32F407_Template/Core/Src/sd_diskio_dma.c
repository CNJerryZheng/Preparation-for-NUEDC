...
DRESULT SD_read(BYTE lun, BYTE* buff, DWORD sector, UINT count)
{
    ...
    if ((buff == NULL) || (count == 0U) || (SD_WaitCardReady(SD_DMA_TIMEOUT_MS) != 0))
    {
        return RES_ERROR;
    }
    ...
    {
        ReadStatus = 0U;
        if ((BSP_SD_ReadBlocks_DMA((uint32_t*)buff, (uint32_t)sector, count) != MSD_OK) || (SD_WaitDma(&ReadStatus, SD_DMA_TIMEOUT_MS) != 0) || (SD_WaitCardReady(SD_DMA_TIMEOUT_MS) != 0))
        {
            return RES_ERROR;
        }
        return RES_OK;
    ...
    for (index = 0U; index < count; ++index)
    {
        ...
        if ((BSP_SD_ReadBlocks_DMA((uint32_t*)ScratchBuffer,
                                   ...
                                   1U)
             != MSD_OK)
            || (SD_WaitDma(&ReadStatus, SD_DMA_TIMEOUT_MS) != 0) || (SD_WaitCardReady(SD_DMA_TIMEOUT_MS) != 0))
        {
            return RES_ERROR;
        }
        memcpy(buff + (index * SD_DEFAULT_BLOCK_SIZE), ScratchBuffer, SD_DEFAULT_BLOCK_SIZE);
    }
...
DRESULT SD_write(BYTE lun, const BYTE* buff, DWORD sector, UINT count)
{
    ...
    {
        WriteStatus = 0U;
        if ((BSP_SD_WriteBlocks_DMA((uint32_t*)buff, (uint32_t)sector, count) != MSD_OK) || (SD_WaitDma(&WriteStatus, SD_DMA_TIMEOUT_MS) != 0) || (SD_WaitCardReady(SD_DMA_TIMEOUT_MS) != 0))
        {
            return RES_ERROR;
        }
        return RES_OK;
    ...
    for (index = 0U; index < count; ++index)
    {
        memcpy(ScratchBuffer, buff + (index * SD_DEFAULT_BLOCK_SIZE), SD_DEFAULT_BLOCK_SIZE);
        WriteStatus = 0U;
        if ((BSP_SD_WriteBlocks_DMA((uint32_t*)ScratchBuffer,
                                    (uint32_t)sector + index,
                                    1U)
             != MSD_OK)
            || (SD_WaitDma(&WriteStatus, SD_DMA_TIMEOUT_MS) != 0) || (SD_WaitCardReady(SD_DMA_TIMEOUT_MS) != 0))
        {
            return RES_ERROR;
        }
    }
