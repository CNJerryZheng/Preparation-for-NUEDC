...
1. 编译工程
2. 使用 probe-rs 下载 `build/MSPA.elf`
3. 复位 MSPM0G3507 并运行程序

也可以分别执行：

```powershell
probe-rs download --chip MSPM0G3507 --verify build/MSPA.elf
...
```powershell
git add Core/MSPA.syscfg Core/Src/ti_msp_dl_config.c Core/Inc/ti_msp_dl_config.h
